...
<section class="bg-pine-50">
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <a href="<?php echo e(route('reviews.index')); ?>" class="w-fit rounded-md bg-white px-4 py-2 text-sm font-semibold text-pine-700 ring-1 ring-pine-200 hover:bg-pine-100">ดูหน้ารีวิว</a>
        </div>
        ...
        <form method="post" action="<?php echo e(route('admin.marketing.reviews.store')); ?>" enctype="multipart/form-data" class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-pine-200">
            <?php echo csrf_field(); ?>
                ...
                <div class="grid gap-4">
                    <div class="grid gap-4 md:grid-cols-2">
                        <label>
                            ...
                            <input name="customer_name" value="<?php echo e(old('customer_name')); ?>" placeholder="สมชาย" class="mt-2 w-full rounded-md border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-pine-200">
                        </label>
                        <label>
                            <span class="text-sm font-semibold text-ink">จังหวัด</span>
                            <input name="province" value="<?php echo e(old('province')); ?>" placeholder="เชียงใหม่" class="mt-2 w-full rounded-md border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-pine-200">
                        </label>
                    </div>
                    <label>
                        ...
                        <select name="rating" class="mt-2 w-full rounded-md border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-pine-200">
                            <?php for($rating = 5; $rating >= 1; $rating--): ?>
                                <option value="<?php echo e($rating); ?>" <?php if((int) old('rating', 5) === $rating): echo 'selected'; endif; ?>><?php echo e(str_repeat('★', $rating)); ?> <?php echo e($rating); ?> ดาว</option>
                            <?php endfor; ?>
                        </select>
                    </label>
                    <label>
                        <span class="text-sm font-semibold text-ink">ข้อความรีวิว</span>
                        <textarea name="review_text" rows="4" placeholder="งานสวยมาก ส่งตรงเวลา" class="mt-2 w-full rounded-md border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-pine-200"><?php echo e(old('review_text')); ?></textarea>
                    </label>
        ...
        <section class="mt-8 rounded-lg bg-white p-6 shadow-sm ring-1 ring-pine-200">
            <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <p class="text-sm text-pine-700">ทั้งหมด <?php echo e(number_format($reviews->count())); ?> รีวิว</p>
            </div>
                                ...
                                <div>
                                    <div class="aspect-[4/3] overflow-hidden rounded-lg bg-[linear-gradient(135deg,#d8b47a,#fff7ed,#b7793b)] ring-1 ring-pine-200">
                                        <?php if($review->image_path): ?>
                                            <img data-review-preview="<?php echo e($review->id); ?>" src="<?php echo e(asset('storage/'.$review->image_path)); ?>" alt="ผลงานของ <?php echo e($review->customer_name); ?>" class="h-full w-full object-cover">
                                        <?php else: ?>
                                            <div data-review-preview="<?php echo e($review->id); ?>" class="flex h-full w-full items-center justify-center text-sm font-semibold text-white/90">ยังไม่มีรูป</div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div>
                                    <form method="post" action="<?php echo e(route('admin.marketing.reviews.update', $review)); ?>" enctype="multipart/form-data" class="grid gap-3">
                                        <?php echo csrf_field(); ?>
                                        <?php echo method_field('patch'); ?>
                                        <div class="grid gap-3 sm:grid-cols-2">
                                            <label>
                                                <span class="text-xs font-semibold text-ink">ชื่อลูกค้า</span>
                                                <input name="customer_name" value="<?php echo e(old('customer_name', $review->customer_name)); ?>" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200">
                                            </label>
                                            <label>
                                                <span class="text-xs font-semibold text-ink">จังหวัด</span>
                                                <input name="province" value="<?php echo e(old('province', $review->province)); ?>" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200">
                                            </label>
                                        </div>
                                        <label>
                                            ...
                                            <select name="rating" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200">
                                                <?php for($rating = 5; $rating >= 1; $rating--): ?>
                                                    <option value="<?php echo e($rating); ?>" <?php if((int) old('rating', $review->rating) === $rating): echo 'selected'; endif; ?>><?php echo e(str_repeat('★', $rating)); ?> <?php echo e($rating); ?> ดาว</option>
                                                <?php endfor; ?>
                                            </select>
                                        </label>
                                        <label>
                                            <span class="text-xs font-semibold text-ink">ข้อความรีวิว</span>
                                            <textarea name="review_text" rows="3" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200"><?php echo e(old('review_text', $review->review_text)); ?></textarea>
                                        </label>
                                        <input type="file" name="image" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" data-review-input="<?php echo e($review->id); ?>" class="block w-full text-sm text-pine-700">
                                        <label class="flex items-center gap-3">
                                            <input type="checkbox" name="active" value="1" <?php if($review->active): echo 'checked'; endif; ?> class="rounded border-pine-300">
                                    ...
                                    <form method="post" action="<?php echo e(route('admin.marketing.reviews.destroy', $review)); ?>" class="mt-3">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', ['title' => 'รีวิวลูกค้า | '.company()->display_name], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\BEER\Documents\Codex\2026-06-17\create-a-laravel-12-project-named\wooden-dad-sales-engine\resources\views\admin\marketing\reviews.blade.php ENDPATH**/ ?>