@extends('layouts.admin', ['title' => 'จัดการผลงาน | '.company()->display_name])
...
<section class="bg-pine-50">
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <a href="{{ route('portfolio.index') }}" class="w-fit rounded-md bg-white px-4 py-2 text-sm font-semibold text-pine-700 ring-1 ring-pine-200 hover:bg-pine-100">ดูหน้า Gallery</a>
        </div>
        ...
        <form method="post" action="{{ route('admin.portfolio.store') }}" enctype="multipart/form-data" class="rounded-lg bg-white p-6 shadow-sm ring-1 ring-pine-200">
            @csrf
            <div class="grid gap-5 lg:grid-cols-[1fr_1fr]">
                <div class="grid gap-4">
                    <label>
                        <span class="text-sm font-semibold text-ink">ชื่อผลงาน</span>
                        <input name="title" value="{{ old('title') }}" placeholder="เช่น ชุดห้องนอนไม้สนบ้านคุณเอ" class="mt-2 w-full rounded-md border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-pine-200">
                    </label>
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <label>
                            ...
                            <input name="sort_order" type="number" min="0" value="{{ old('sort_order', 0) }}" class="mt-2 w-full rounded-md border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-pine-200">
                        </label>
                    </div>
        ...
        <section class="mt-8 rounded-lg bg-white p-6 shadow-sm ring-1 ring-pine-200">
            <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <p class="text-sm text-pine-700">ทั้งหมด {{ number_format($portfolioImages->count()) }} รูป</p>
            </div>
                ...
                <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    @foreach ($portfolioImages as $image)
                        <article class="overflow-hidden rounded-lg bg-pine-50 ring-1 ring-pine-200">
                            <img src="{{ asset('storage/'.$image->image_path) }}" alt="{{ $image->title ?: $image->category_name }}" class="aspect-[4/3] w-full object-cover">
                            <div class="p-4">
                                <form method="post" action="{{ route('admin.portfolio.update', $image) }}" class="grid gap-3">
                                    @csrf
                                    @method('patch')
                                    <label>
                                        <span class="text-xs font-semibold text-ink">ชื่อผลงาน</span>
                                        <input name="title" value="{{ old('title', $image->title) }}" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200">
                                    </label>
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <label>
                                            ...
                                            <select name="category" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200">
                                                @foreach ($categories as $key => $label)
                                                    <option value="{{ $key }}" @selected(old('category', $image->category) === $key)>{{ $label }}</option>
                                                @endforeach
                                            </select>
                                        </label>
                                        <label>
                                            <span class="text-xs font-semibold text-ink">ลำดับ</span>
                                            <input name="sort_order" type="number" min="0" value="{{ old('sort_order', $image->sort_order) }}" class="mt-1 w-full rounded-md border-0 bg-white px-3 py-2 text-sm ring-1 ring-pine-200">
                                        </label>
                                    </div>
                                    <label class="flex items-center gap-3">
                                        <input type="checkbox" name="active" value="1" @checked($image->active) class="rounded border-pine-300">
                                ...
                                <form method="post" action="{{ route('admin.portfolio.destroy', $image) }}" class="mt-3">
                                    @csrf
                                    @method('delete')
