        ...
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($stages as $status => $label)
                    <div class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-pine-200">
                        <dt class="text-sm font-medium text-pine-700">{{ $label }}</dt>
                        <dd class="mt-2 text-2xl font-semibold text-ink">{{ number_format($counts[$status] ?? 0) }}</dd>
                    </div>
                @endforeach
            </dl>
            ...
            <section class="mt-8 rounded-lg bg-white p-5 shadow-sm ring-1 ring-pine-200" data-production-board>
                ...
                <div class="mt-5 grid gap-4 xl:grid-cols-7">
                    @foreach ($stages as $status => $label)
                        <div class="rounded-lg bg-pine-50 p-3 ring-1 ring-pine-200" data-production-column data-status="{{ $status }}">
                            <div class="flex items-center justify-between gap-3">
                                <h3 class="text-sm font-semibold text-ink">{{ $label }}</h3>
                                <span class="rounded-full bg-white px-2 py-1 text-xs font-semibold text-pine-700 ring-1 ring-pine-200" data-stage-count>{{ ($ordersByStage[$status] ?? collect())->count() }}</span>
                            </div>
                            <div class="mt-4 min-h-36 space-y-3 rounded-md border border-dashed border-transparent p-1 transition" data-production-dropzone>
                                @forelse (($ordersByStage[$status] ?? collect()) as $order)
                                    <article
                                        ...
                                        data-update-url="{{ route('admin.production.status', $order) }}"
                                    >
                                        <a href="{{ route('admin.production.show', $order) }}" class="block text-sm font-semibold text-ink underline-offset-4 hover:text-pine-700 hover:underline">{{ $order->production_order_number }}</a>
                                        <p class="mt-1 truncate text-xs text-pine-700">{{ $order->lead->name }} · {{ $order->quotation->quotation_number }}</p>
                                        <p class="mt-2 text-xs font-semibold text-pine-700">฿{{ number_format((float) $order->quotation->subtotal, 2) }}</p>
                                        <div class="mt-3 flex flex-wrap gap-1">
                                            @forelse ($order->craftsmen as $craftsman)
                                                <span class="rounded-full bg-pine-100 px-2 py-1 text-xs font-semibold text-pine-700">{{ $craftsman->name }}</span>
                                            @empty
                                                <span class="rounded-full bg-rose-50 px-2 py-1 text-xs font-semibold text-rose-700">ยังไม่ระบุช่าง</span>
                                            @endforelse
                                        </div>
                                        <a href="{{ route('admin.production.show', $order) }}" class="mt-3 inline-flex rounded-md bg-pine-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-pine-500">เปิดงาน</a>
                                    </article>
                                @empty
