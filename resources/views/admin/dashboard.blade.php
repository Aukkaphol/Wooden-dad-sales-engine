@extends('layouts.admin', ['title' => 'แดชบอร์ด Sales CRM | '.company()->display_name])
    ...
    <section class="bg-pine-50">
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <a href="{{ route('admin.leads.index') }}" class="inline-flex w-fit rounded-xl bg-pine-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-pine-500">เปิด CRM Pipeline</a>
            </div>
            ...
            <dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    ...
                    <dd class="mt-2 text-3xl font-semibold text-blue-700">{{ number_format($metrics['website_leads']) }}</dd>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    <dt class="text-sm font-medium text-pine-700">Facebook Leads</dt>
                    <dd class="mt-2 text-3xl font-semibold text-indigo-700">{{ number_format($metrics['facebook_leads']) }}</dd>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    <dt class="text-sm font-medium text-pine-700">LINE Leads</dt>
                    <dd class="mt-2 text-3xl font-semibold text-emerald-700">{{ number_format($metrics['line_leads']) }}</dd>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                ...
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    ...
                    <dd class="mt-2 text-3xl font-semibold text-orange-700">{{ number_format($metrics['active_production']) }}</dd>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    <dt class="text-sm font-medium text-pine-700">Sales This Month</dt>
                    <dd class="mt-2 text-2xl font-semibold text-ink">฿{{ number_format($metrics['sales_this_month'], 2) }}</dd>
                </div>
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    <dt class="text-sm font-medium text-pine-700">Conversion Rate</dt>
                    <dd class="mt-2 text-3xl font-semibold text-green-700">{{ number_format($metrics['conversion_rate'], 1) }}%</dd>
                </div>
            </dl>
            ...
            <div class="mt-8 grid gap-6 xl:grid-cols-[1.2fr_.8fr]">
                        ...
                        @foreach ($leadGrowth as $row)
                            <div class="flex min-w-14 flex-1 flex-col items-center gap-2">
                                <div class="flex h-44 w-full items-end rounded-xl bg-pine-50 px-2">
                                    <div class="w-full rounded-t-xl bg-pine-600" style="height: {{ max(6, ($row['value'] / $leadMax) * 100) }}%"></div>
                                </div>
                                <div class="text-center">
                                    <p class="text-sm font-semibold text-ink">{{ number_format($row['value']) }}</p>
                                    <p class="text-[11px] text-pine-600">{{ $row['month'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-pine-200">
                    ...
                    <div class="mt-6 space-y-4">
                        @foreach ($conversionByMonth as $row)
                            <div>
                                <div class="mb-1 flex justify-between gap-3 text-sm">
                                    <span class="font-medium text-pine-700">{{ $row['month'] }}</span>
                                    <span class="font-semibold text-ink">{{ number_format($row['value'], 1) }}%</span>
                                </div>
                                <div class="h-3 rounded-full bg-pine-100">
                                    <div class="h-3 rounded-full bg-emerald-500" style="width: {{ min(100, max(4, ($row['value'] / $conversionMax) * 100)) }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="mt-8 grid gap-6 xl:grid-cols-2">
                <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-pine-200">
                    ...
                    <div class="mt-6 space-y-5">
                        @foreach ($leadSourceMonths as $row)
                            @php
                            ...
                            <div>
                                ...
                                <div class="flex h-4 overflow-hidden rounded-full bg-pine-100">
                                    <div class="bg-blue-500" style="width: {{ $total > 0 ? ($row['website'] / $sourceMax) * 100 : 0 }}%"></div>
                                    <div class="bg-indigo-500" style="width: {{ $total > 0 ? ($row['facebook'] / $sourceMax) * 100 : 0 }}%"></div>
                                    <div class="bg-emerald-500" style="width: {{ $total > 0 ? ($row['line'] / $sourceMax) * 100 : 0 }}%"></div>
                                </div>
                ...
                <section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-pine-200">
                    ...
                    <div class="mt-6 space-y-4">
                        @foreach ($salesForecast as $row)
                            <div>
                                <div class="mb-1 flex justify-between gap-3 text-sm">
                                    <span class="font-medium text-pine-700">{{ $row['month'] }}</span>
                                    <span class="font-semibold text-ink">฿{{ number_format($row['value'], 2) }}</span>
                                </div>
                                <div class="h-3 rounded-full bg-pine-100">
                                    <div class="h-3 rounded-full bg-amber-500" style="width: {{ max(4, ($row['value'] / $forecastMax) * 100) }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
