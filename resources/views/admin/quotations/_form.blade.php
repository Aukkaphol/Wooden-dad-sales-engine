...
<form action="{{ $action }}" method="post" class="space-y-6" data-quotation-form>
    @csrf
    @if ($method !== 'POST')
        @method($method)
    @endif
    ...
    <section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200 sm:p-6">
            ...
            <label class="lg:col-span-1">
                ...
                <select name="lead_id" required data-lead-select class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    ...
                    @foreach ($leads as $row)
                        <option value="{{ $row->id }}" @selected((string) $selectedLead === (string) $row->id)>{{ $row->name }} | {{ $row->phone }}</option>
                    @endforeach
                </select>
            </label>
            <label>
                <span class="text-sm font-semibold text-ink">ชื่อลูกค้า</span>
                <input name="customer_name" value="{{ old('customer_name', $quotation->customer_name ?? $lead?->name) }}" required data-customer-name class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
            </label>
            <label>
                <span class="text-sm font-semibold text-ink">เบอร์โทร</span>
                <input name="phone" value="{{ old('phone', $quotation->phone ?? $lead?->phone) }}" required data-phone class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
            </label>
            <label>
                <span class="text-sm font-semibold text-ink">จังหวัด</span>
                <input name="province" value="{{ old('province', $quotation->province ?? $lead?->province) }}" required data-province class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
            </label>
            <label>
                <span class="text-sm font-semibold text-ink">ชื่องาน / โครงการ</span>
                <input name="project_name" value="{{ old('project_name', $quotation->project_name ?? 'เฟอร์นิเจอร์ไม้สนสั่งทำ') }}" class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
            </label>
            <label>
        ...
        <div class="mt-5 grid gap-3 rounded-xl bg-pine-50 p-4 text-sm text-pine-700 sm:grid-cols-3">
            <p>งบประมาณ: <span class="font-semibold text-ink" data-budget>{{ $lead?->budget ?: '-' }}</span></p>
            <p>กว้าง: <span class="font-semibold text-ink" data-room-width>{{ $lead?->room_width ?: '-' }}</span></p>
            <p>ยาว: <span class="font-semibold text-ink" data-room-length>{{ $lead?->room_length ?: '-' }}</span></p>
        </div>
    </section>
            ...
            @foreach ($formItems as $index => $item)
                <div class="grid gap-3 rounded-2xl bg-pine-50 p-4 lg:grid-cols-[1.1fr_1.2fr_90px_90px_130px_130px_auto]" data-item-row>
                    <label>
                        ...
                        <input name="items[{{ $index }}][item_name]" value="{{ $item['item_name'] ?? '' }}" required class="mt-1 w-full rounded-xl border-0 bg-white px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <label>
                        <span class="text-xs font-semibold text-pine-700">Description</span>
                        <input name="items[{{ $index }}][description]" value="{{ $item['description'] ?? '' }}" class="mt-1 w-full rounded-xl border-0 bg-white px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <label>
                        <span class="text-xs font-semibold text-pine-700">Qty</span>
                        <input name="items[{{ $index }}][qty]" type="number" step="0.01" min="0.01" value="{{ $item['qty'] ?? 1 }}" required data-qty class="mt-1 w-full rounded-xl border-0 bg-white px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <label>
                        <span class="text-xs font-semibold text-pine-700">Unit</span>
                        <input name="items[{{ $index }}][unit]" value="{{ $item['unit'] ?? 'ชุด' }}" required class="mt-1 w-full rounded-xl border-0 bg-white px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <label>
                        <span class="text-xs font-semibold text-pine-700">Unit Price</span>
                        <input name="items[{{ $index }}][unit_price]" type="number" step="0.01" min="0" value="{{ $item['unit_price'] ?? 0 }}" required data-unit-price class="mt-1 w-full rounded-xl border-0 bg-white px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <div>
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200 sm:p-6">
            <div class="grid gap-5 sm:grid-cols-2">
                <label>
                    ...
                    <input name="valid_until" type="date" value="{{ old('valid_until', optional($quotation?->valid_until)->format('Y-m-d')) }}" class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                </label>
                <label>
                    <span class="text-sm font-semibold text-ink">หมายเหตุ</span>
                    <input name="remark" value="{{ old('remark', $quotation->remark ?? $quotation->notes ?? '') }}" class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-3 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                </label>
            </div>
        </div>
        ...
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200 sm:p-6">
            ...
            <div class="mt-4 space-y-3">
                ...
                <label class="flex items-center justify-between gap-4">
                    ...
                    <input name="discount" type="number" step="0.01" min="0" value="{{ old('discount', $quotation->discount ?? 0) }}" data-discount class="w-36 rounded-xl border-0 bg-pine-50 px-3 py-2 text-right text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                </label>
                <label class="flex items-center justify-between gap-4">
                    <span class="text-sm text-pine-700">Shipping</span>
                    <input name="shipping_cost" type="number" step="0.01" min="0" value="{{ old('shipping_cost', $quotation->shipping_cost ?? 0) }}" data-shipping class="w-36 rounded-xl border-0 bg-pine-50 px-3 py-2 text-right text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                </label>
                <label class="flex items-center justify-between gap-4">
                    <span class="text-sm text-pine-700">Deposit</span>
                    <input name="deposit_amount" type="number" step="0.01" min="0" value="{{ old('deposit_amount', $quotation->deposit_amount ?? 0) }}" data-deposit class="w-36 rounded-xl border-0 bg-pine-50 px-3 py-2 text-right text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                </label>
            </div>
            ...
            <button class="mt-6 w-full rounded-xl bg-pine-700 px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-pine-500">{{ $submitLabel }}</button>
        </div>
    </section>
</form>
