@extends('layouts.admin', ['title' => 'CRM Pipeline | '.company()->display_name])
    ...
    <section class="bg-pine-50">
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <a href="{{ route('admin.leads.export', request()->query()) }}" class="inline-flex w-fit items-center justify-center rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-pine-700 shadow-sm ring-1 ring-pine-200 hover:bg-pine-100">ส่งออก Excel</a>
            </div>

            @if (session('success'))
                <div class="mt-6 rounded-xl bg-green-50 p-4 text-sm font-medium text-green-800 ring-1 ring-green-600/20">{{ session('success') }}</div>
            @endif
            ...
            <dl class="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                    ...
                    <dd class="mt-2 text-3xl font-semibold text-green-700">{{ number_format($kpis['conversion_rate'], 1) }}%</dd>
                </div>
            </dl>
                    ...
                    @forelse ($monthlyLeads as $month)
                        <div class="flex min-w-14 flex-1 flex-col items-center gap-2">
                            <div class="flex h-32 w-full items-end rounded-xl bg-pine-50 px-2">
                                <div class="w-full rounded-t-xl bg-pine-600" style="height: {{ max(6, ($month->total / $maxMonthly) * 100) }}%"></div>
                            </div>
                            <div class="text-center">
                                <p class="text-sm font-semibold text-ink">{{ number_format($month->total) }}</p>
                                <p class="text-[11px] text-pine-600">{{ $month->month }}</p>
                            </div>
                        </div>
                    @empty
            ...
            <section class="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200" data-kanban-board>
                ...
                <div class="mt-5 grid gap-4 xl:grid-cols-4">
                    @foreach ($kanbanStages as $stage => $label)
                        <div class="rounded-xl bg-pine-50 p-3 ring-1 ring-pine-200" data-kanban-column data-stage="{{ $stage }}">
                            ...
                            <div class="mt-4 min-h-32 space-y-3 rounded-xl border border-dashed border-transparent p-1 transition" data-kanban-dropzone>
                                @forelse (($kanbanLeads[$stage] ?? collect()) as $lead)
                                    <article class="cursor-grab rounded-xl bg-white p-3 shadow-sm ring-1 ring-pine-200 transition active:cursor-grabbing" draggable="true" data-lead-card data-lead-id="{{ $lead->id }}" data-update-url="{{ route('admin.leads.status', $lead) }}">
                                        <div class="flex items-start justify-between gap-3">
                                            ...
                                            <span class="shrink-0 rounded-full bg-pine-100 px-2 py-1 text-[11px] font-semibold text-pine-700">{{ $lead->source_label }}</span>
                                        </div>
                                        ...
                                        <form action="{{ route('admin.leads.status', $lead) }}" method="post" class="mt-3 flex gap-2">
                                            ...
                                            <select name="lead_status" data-card-status-select class="min-w-0 flex-1 rounded-lg border-0 bg-pine-50 px-2 py-1.5 text-xs ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                                                @foreach ($kanbanStages as $value => $stageLabel)
                                                    <option value="{{ $value }}" @selected(($lead->status ?: $lead->lead_status) === $value)>{{ $stageLabel }}</option>
                                                @endforeach
                                            </select>
            ...
            <form method="get" action="{{ route('admin.leads.index') }}" class="mt-8 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-7">
                    <label class="xl:col-span-2">
                        ...
                        <input name="search" value="{{ $filters['search'] ?? '' }}" placeholder="เช่น สมชาย หรือ 081..." class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 placeholder:text-pine-400 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <label>
                    ...
                    <label>
                        ...
                        <input name="date_from" type="date" value="{{ $filters['date_from'] ?? '' }}" class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                    <label>
                        <span class="text-sm font-semibold text-ink">ถึงวันที่</span>
                        <input name="date_to" type="date" value="{{ $filters['date_to'] ?? '' }}" class="mt-2 w-full rounded-xl border-0 bg-pine-50 px-3 py-2.5 text-sm ring-1 ring-inset ring-pine-200 focus:ring-2 focus:ring-pine-500">
                    </label>
                </div>
                <div class="mt-5 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('admin.leads.index') }}" class="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-pine-700 ring-1 ring-inset ring-pine-200 hover:bg-pine-100">ล้างตัวกรอง</a>
                </div>
            </form>
                    ...
                    <tbody class="divide-y divide-pine-100 bg-white">
                        @forelse ($leads as $lead)
                            <tr class="hover:bg-pine-50">
                                <td class="px-4 py-4">
                                    <div class="truncate font-semibold text-ink">{{ $lead->name }}</div>
                                    <div class="mt-1 truncate text-sm text-pine-700">{{ $lead->phone ?: '-' }}</div>
                                </td>
                                <td class="px-4 py-4 text-sm font-semibold text-pine-700">{{ $lead->source_label }}</td>
                                <td class="px-4 py-4 text-sm text-pine-700">{{ $lead->province ?: '-' }}</td>
                                <td class="px-4 py-4 text-sm text-pine-700">{{ $lead->budget ?: '-' }}</td>
                                <td class="px-4 py-4">
                                    <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset {{ $statusClasses[$lead->status ?: $lead->lead_status] ?? 'bg-pine-100 text-pine-700 ring-pine-600/20' }}">{{ $lead->lead_status_label }}</span>
                                </td>
                                <td class="px-4 py-4 text-sm text-pine-700">{{ $lead->created_at->format('d/m/Y H:i') }}</td>
                                <td class="px-4 py-4 text-right">
                                    <a href="{{ route('admin.leads.show', ['lead' => $lead] + request()->query()) }}" class="text-sm font-semibold text-pine-700 hover:text-ink">เปิดดู</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                @forelse ($leads as $lead)
                    <article class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-pine-200">
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                <h2 class="truncate font-semibold text-ink">{{ $lead->name }}</h2>
                                <p class="mt-1 text-sm text-pine-700">{{ $lead->phone ?: '-' }} · {{ $lead->province ?: '-' }}</p>
                                <p class="mt-1 text-xs font-semibold text-pine-700">{{ $lead->source_label }}</p>
                            </div>
                            <span class="inline-flex shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset {{ $statusClasses[$lead->status ?: $lead->lead_status] ?? 'bg-pine-100 text-pine-700 ring-pine-600/20' }}">{{ $lead->lead_status_label }}</span>
                        </div>
                        <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div>
                                ...
                                <p class="font-medium text-ink">{{ $lead->created_at->format('d/m/Y') }}</p>
                            </div>
                        </div>
                        <div class="mt-4 flex justify-end">
                            <a href="{{ route('admin.leads.show', ['lead' => $lead] + request()->query()) }}" class="text-sm font-semibold text-pine-700">เปิดดูรายละเอียด</a>
                        </div>
                    </article>
                @empty
